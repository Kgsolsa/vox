(function () {
  const bilu = {
    init: function (config) {
      ...
      // Clear previous content to avoid duplicates if init() is called multiple times
      ...
      // Loader HTML with identifiable class for removal
      const loaderHtml = `
        <div class="loader-spinner flex justify-center p-6" role="status">
      ...
      formContainer.appendChild(commentsList);

      commentDiv.appendChild(formContainer);

      // Loader functions
      function showLoader() {
        commentsList.innerHTML = loaderHtml;
      }
      function hideLoader() {
        const loader = commentsList.querySelector('.loader-spinner');
        ...
        fetch(config.api + "/comments?url=" + encodeURIComponent(config.url) + "&t=" + timestamp)
          ...
          .then(data => {
            ...
            if (!Array.isArray(data) || data.length === 0) {
              commentsList.innerHTML = '<p class="text-sm text-gray-500">No comments yet. Be the first!</p>';
              return;
            }
            const commentsHtml = data.map(comment =>
              `<div class="p-2 border-b font-inconsolata text-sm">
                <p id="${escapeHtml(comment.id)}"><strong class="text-gray-800">${escapeHtml(comment.user)}</strong></p>
                <p class="text-sm text-gray-600">${escapeHtml(comment.comment)}</p>
                <span class="text-xs text-gray-500">${comment.timestamp ? new Date(comment.timestamp).toLocaleString() : ""}</span>
              </div>`
            ).join('');
